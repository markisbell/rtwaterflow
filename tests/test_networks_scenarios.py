...
def test_networks_list_and_preview():
    with make_api_client() as client:
        ...
        assert {"tutorial_hillside", "musterdorf", "mustertal"} <= ids
        ...
        assert md["nodes"] == 35
        assert md["pipe_km"] == pytest.approx(5.05, abs=0.01)
        prev = client.get("/networks/musterdorf").json()
        assert prev["n_pipes"] == 33
        ...
        # M2: the ext_grid moved down to the Wasserwerk (the Hochbehälter
        # became a level-integrating tank)
        assert prev["supply"]["node"] == "ww"
